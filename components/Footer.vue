<template>
  <footer>
    <ul>
      <li><h3>Authors</h3></li>
      <li>//TBC</li>
      <li>//TBC</li>
    </ul>
    <ul>
      <li><h3>Original Authors (Thanks folks!)</h3></li>
      <li><a href="https://keybase.io/shazow">shazow</a></li>
      <li><a href="https://keybase.io/mveytsman">mveytsman</a></li>
    </ul>
    <ul>
      <li><h3>Project</h3></li>
      <li><NuxtLink to="faq">FAQ</NuxtLink></li>
      <li><NuxtLink to="press">Press</NuxtLink></li>
      <li><a href="https://github.com/0xcrtnmnr/msolhomepage">Source code</a></li>
      <!--<li><a href="https://twitter.com/msolhomepage">Twitter</a></li>
      <li><a href="https://discord.gg/PXVRw37bQ7">Discord</a></li>
      <li><a href="https://opensea.io/collection/thousand-ether-homepage">OpenSea</a></li>-->
    </ul>
    <slot></slot>
  </footer>
</template>
