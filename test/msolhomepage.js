...
describe('MsolHomepage', function() {
  let MsolHomepage;
  ...
  beforeEach(async function () {
    MS = await MsolHomepage.deploy(owner.getAddress(), withdrawWallet.getAddress());
  });

  it("should have correct constants by default", async function() {
    const wpp = await MS.weiPixelPrice();
    expect(weiPixelPrice).to.equal(wpp);

    const ppc = await MS.pixelsPerCell();
  ...
  it("should let users buy if they send enough eth", async function() {
    const result = await MS.connect(account1).buy(0, 0, 10, 10, { value: oneHundredCellPrice });
    const receipt = await result.wait(); // Let events populate

    const events = await MS.queryFilter(MS.filters.Buy(), receipt.blockHash)
    ...
    expect(await MS.grid(0, 0)).to.be.true;
    expect(await MS.grid(10, 0)).to.be.false;

    const ads = await MS.ads(idx);
  ...
  it("shouldn't let users buy overlapping adspace", async function() {
    await MS.connect(account1).buy(0, 0, 10, 10, { value: oneHundredCellPrice });
    await expect(
      MS.connect(account2).buy(5, 5, 10, 10, { value: oneHundredCellPrice })
    ).to.be.reverted
  });

  it("should let a user publish their own ad", async function() {
    await MS.connect(account1).buy(0, 0, 10, 10, { value: oneHundredCellPrice });
    await MS.connect(account1).publish(0, "link", "image", "title", false)
    const ad = await MS.ads(0);
  ...
  it("shouldn't let a user publish another user's ad", async function() {
    await MS.connect(account1).buy(0, 0, 10, 10, { value: oneHundredCellPrice });
    await expect(
      MS.connect(account2).publish(0, "link", "image", "title", false)
    ).to.be.reverted
  });

  it("should let the contract owner forceNSFW", async function() {
    await MS.connect(account1).buy(0, 0, 10, 10, { value: oneHundredCellPrice });
    await MS.connect(owner).forceNSFW(0, true);
    const ad = await MS.ads(0);
  ...
  it("shouldn't let non contract owners forceNSFW", async function() {
    await MS.connect(account1).buy(0, 0, 10, 10, { value: oneHundredCellPrice });
    await expect(
      MS.connect(account1).forceNSFW(0, true)
    ).to.be.reverted
  });

  it("should let the owner withdraw", async function() {
    const tx = await MS.connect(account1).buy(0, 0, 10, 10, { value: oneHundredCellPrice });
    const receipt = await tx.wait();
    const initialBalance = await withdrawWallet.getBalance();

    await MS.connect(owner).withdraw();
  ...
  it("shouldn't let non-owners withdraw", async function() {
    await MS.connect(account1).buy(0, 0, 10, 10, { value: oneHundredCellPrice });
    await expect(
      MS.connect(account1).withdraw()
    ).to.be.reverted
  });

  it("shouldn't let users buy 0-width space", async function() {
    await expect(
      MS.connect(account1).buy(0, 0, 10, 0, { value: oneHundredCellPrice })
    ).to.be.reverted
  });

  it("shouldn't let users buy negative space", async function() {
    await expect(
      MS.connect(account1).buy(30, 30, -10, -10, { value: oneHundredCellPrice })
    ).to.be.reverted
  });

  it("shouldn't let users buy out of bounds space", async function() {
    await expect(
      MS.connect(account1).buy(99, 99, 10, 10, { value: oneHundredCellPrice })
    ).to.be.reverted
  });
  ...
  it("should let a user setAdOwner on their own ad", async function() {
    ...
    const result = await MS.connect(account1).setAdOwner(0, await account2.getAddress());
    const receipt = await result.wait();

    const events = await MS.queryFilter(MS.filters.SetAdOwner(), receipt.blockHash)
  ...
  it("shouldn't let a user setAdOwner on another user's ad", async function() {
    ...
    await expect(
      MS.connect(account2).setAdOwner(0, await account2.getAddress())
    ).to.be.reverted;
  });
});
